...
class BaseInformationFragment : Fragment() {
    ...
    private fun initBind() {
        ...
        val question4 = OnboardQuestion(
            "룸메와 연락할 개인 오픈채팅 링크를 알려주세요.",
            "(필수)",
            View.GONE,
            300,
            placeholder = "URL이 포함된 메시지를 붙여넣어도 돼요."
        )
        ...
        val list = arrayListOf<OnboardQuestion>(
            question1,
            question2,
            question3,
            question4,
    ...
    private fun checkInfo(
        ...
        val url = urlExtractor(openKakaoLink)
        if (age < 20) {
        ...
        IDormLogger.d(this, hostExtractor(url ?: ""))
        if (url.isNullOrEmpty() || hostExtractor(url) != "open.kakao.com") {
            OkDialog("오픈채팅 링크가 올바르지 않습니다.").show(requireContext())
            return false
        }
                    ...
                    is BaseInfoMutationEvent.CreateBaseInfo -> {
                        if (it.mutation.state.isSuccess()) {
                            OkDialog("매칭 이미지가 저장되었습니다.", onOk = {
                                val intent =
                                    Intent(requireContext(), MyMatchingProfileActivity::class.java)
